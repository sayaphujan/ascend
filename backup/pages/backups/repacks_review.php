    ...
    tabel_delivered = $('#delivered_list').DataTable({
        ...
        "columns": [
            ...
            { "data": "next_repack", "render": function ( data, type, row, meta ){
                var today = new Date();
                var day_before = new Date(row.next_repack);
                day_before.setDate(day_before.getDate() - 5);
                today.setHours(0, 0, 0, 0);
                
                console.log(day_before+'<br/>'+today);
                
                if (day_before.getTime() === today.getTime()) {
                  return '<div style="text-align: center">' + data + '</div><br/><div class="alert alert-warning align-items-center next_repack_alert" role="alert">5 days until next repack</div>';
                } else {
                  return '<div style="text-align: center">' + data + '</div>';
                }
              }
